	...
	<table class="table table-striped table-bordered table-hover">
		...
		<tbody>
			...
			<tr id="column_{COLUMN.column_name}">
				...
				<td>
				<select name="views[{COLUMN.column_name}]" onchange="nv_change('{COLUMN.column_name}');">
					<!-- BEGIN: field_type -->
			...
			<tr id="{COLUMN.column_name}">
				...
				<table class="table table-striped table-bordered table-hover" id="choicesql_{COLUMN.column_name}">
					...
					<tbody>
						<tr>
							<td><span id="choicesql_module_{COLUMN.column_name}">&nbsp;</span></td>
							<td><span id="choicesql_table_{COLUMN.column_name}">&nbsp;</span></td>
							<td><span id="choicesql_column_{COLUMN.column_name}">&nbsp;</span></td>
						</tr>
					</tbody>
				</table>
				<table class="table table-striped table-bordered table-hover" id="choiceitems_{COLUMN.column_name}" >
					...
					<tfoot>
						<tr>
							<td colspan="4" ><input style="margin-left: 50px;" type="button" value="{LANG.field_add_choice}" onclick="nv_choice_fields_additem('{COLUMN.column_name}');" /></td>
						</tr>
					</tfoot>
					<tbody>
						...
						<tr class="text-center">
							...
							<td><input class="form-control w100 validalphanumeric" type="text" value="{FIELD_CHOICES.key}" name="field_choice[{COLUMN.column_name}][{FIELD_CHOICES.number}]" /></td>
							<td><input class="form-control w350" type="text" value="{FIELD_CHOICES.value}" name="field_choice_text[{COLUMN.column_name}][{FIELD_CHOICES.number}]" /></td>
							<td><input type="radio" {FIELD_CHOICES.checked} value="{FIELD_CHOICES.number}" name="default_value_choice[{COLUMN.column_name}]"></td>
						</tr>
			...
			<script type="text/javascript">
				...
				function nv_load_sqlchoice(choice_name_select, choice_seltected, column) {
					...
					$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=addfun&nocache=' + new Date().getTime(), 'choicesql=1&choice=' + choice_name_select + getval + '&choice_seltected=' + choice_seltected + '&column=' + column, function(res) {
	...
	<table class="table table-striped table-bordered table-hover">
		...
		<tbody>
			...
			<tr>
				...
				<td><input type="checkbox" name="setfunction" value="1" {SETFUNCTION_CHECKED}/> (Mặc định là admin)</td>
				<td>Chức năng kích hoạt</td>
				<td>
				<select name="active_page">
		...
		<tfoot>
			<tr>
				<td align="center" colspan="4"><input class="btn btn-primary" type="submit" value="Thực hiện" /></td>
			</tr>
		</tfoot>
	</table>
</form>
