...
function nv_write_lang_mod_admin( $mod, $lang, $arr_new_lang, $setfunction )
{
	...
	if( ! empty( $arr_new_lang ) )
	{
		...
		file_put_contents( NV_ROOTDIR . '/modules/' . $mod . '/language/' . $file_name . $lang . '.php', $content_lang, LOCK_EX );
	}
}
...
$setfunction = $nv_Request->get_int( 'setfunction', 'get,post', 0 );
...
$xtpl->assign( 'SETFUNCTION', $setfunction );
	...
	try
	{
		...
		if( ! empty( $array_views ) )
		{
			...
			if( $setfunction == 1 )
			{
				...
				$content .= "if( ! defined( 'NV_IS_MOD_" . $modname_up . "' ) ) die( 'Stop!!!' );\n\n";
			...
			{
				if( ! empty( $active_page ) ) // neu co cot active
				{
			...
			if( $type_addfun == 0 or $type_addfun == 1 )
			{
				...
				$content .= "\t\t\$nv_Cache->delMod( \$module_name );\n";
				$content .= "\t\tHeader( 'Location: ' . " . $nv_url . " . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . \$module_name . '&' . NV_OP_VARIABLE . '=' . \$op );\n";
			...
			file_put_contents( NV_ROOTDIR . "/modules/" . $modname . "/" . $folder . "/" . $funname . ".php", $content, LOCK_EX );
			if( substr( $sys_info['os'], 0, 3 ) != 'WIN' ) chmod( NV_ROOTDIR . "/modules/" . $modname . "/" . $folder . "/" . $funname . ".php", 0777 );
			...
			file_put_contents( NV_ROOTDIR . "/themes/" . $folder . "/modules/" . $modname . "/" . $funname . ".tpl", $content_1 . $content_2, LOCK_EX );
			if( substr( $sys_info['os'], 0, 3 ) != 'WIN' ) chmod( NV_ROOTDIR . "/themes/" . $folder . "/modules/" . $modname . "/" . $funname . ".tpl", 0777 );
